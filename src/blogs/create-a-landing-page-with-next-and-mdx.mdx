...
For a sneek peek at what we will build head on over to [next-mdx-landing-page.vercel.app/](https://next-mdx-landing-page.vercel.app/)
...
Finally we need to wrap our MDX in our new Layout component.
...
So now that we have all of our pieces lets put them together and see how simple it can be to re-create that poster!
...
As you can see, we are utilizing some of the stuff we have learnt along the way such as our grid column syntax and the responsive array syntax. Its not a perfect match to the poster but I believe that our client would be mightily impressed with our work!

Now I know what you might be thinking.... You haven't used any markdown syntax, this could have just been a regular JSX file. And you would be correct in thinking that! Buuuuuuut. What we have above is a collection of generic(ish) components which can be used both in MDX files and normal JSX. If we wanted we could just start writing straight in the markup whatever we wanted and it would map to our components passed into the MDXProvider and be styled the way we want. We could re-use our Section component again and create a whole new section of the page that is pure markdown if we wanted, or we could create a new JSX page and use the same components, if we were that way inclined.
